...
I want to build a very simple dice roller mcp server. I want to do coin flips, dnd stuff, any kind of dice roller mechanic, bake that in. Make it simple, clean
...
1. **NO `@mcp.prompt()` decorators** - They break VsCode

2. **NO `prompt` parameter to FastMCP()** - It breaks VsCode
...
VsCode → MCP Gateway → [SERVICE_NAME] MCP Server → [SERVICE/API]
...
- Ensure VsCode config includes custom catalog
...
- [ ] VsCode config JSON has no comments
